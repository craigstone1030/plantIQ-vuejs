  ...
  val => {
    if (val) {
      const alert = store.getAlerts.find(
        i => i.pk === store.getSelectedAlertId
      );
      form.name = alert.fields.name;
      form.description = alert.fields.description;
      form.treshold1 = alert.fields.nearCriticalTreshold;
      form.treshold2 = alert.fields.criticalTreshold;
      form.duration = alert.fields.duration;
    }
  }
);
</script>
